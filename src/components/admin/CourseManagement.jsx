...
const CourseCard = ({ course, onDelete, onEdit, onManageBatches, loading }) => {
      ...
      <div className="p-6">
        ...
        <div className="mb-4">
          <div className="flex justify-between text-xs text-gray-500 mb-1">
            ...
            <span>{Math.min((course.enrolledStudents || 0) * 2, 100)}%</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div 
              className="bg-gradient-to-r from-green-500 to-emerald-400 h-2 rounded-full transition-all duration-500"
              style={{ width: `${Math.min((course.enrolledStudents || 0) * 2, 100)}%` }}
            ></div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button 
            ...
            className="bg-gradient-to-r from-indigo-500 to-purple-600 text-white py-2 px-3 rounded-xl text-sm font-medium hover:shadow-lg transition-all duration-300 hover:scale-105"
          >
            Edit
          </button>
          <button 
            onClick={() => onManageBatches(course)}
            className="bg-gradient-to-r from-blue-500 to-cyan-600 text-white py-2 px-3 rounded-xl text-sm font-medium hover:shadow-lg transition-all duration-300 hover:scale-105"
          >
            Batches
          </button>
          <button 
            ...
            disabled={loading}
            className="bg-gradient-to-r from-red-500 to-pink-600 text-white py-2 px-3 rounded-xl text-sm font-medium hover:shadow-lg transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed col-span-2"
          >
...
const BatchManagementModal = ({ course, onClose, onBatchUpdate }) => {
  ...
  const addBatch = () => {
    if (batchForm.name && batchForm.startDate) {
      const newBatch = {
        ...batchForm,
        ...
        <div className="p-6">
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <button
              ...
              className="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-4 py-2 rounded-xl font-medium hover:shadow-lg transition-all"
            >
              + Add Batch
            </button>
          </div>
          ...
          {showBatchForm && (
            <div className="bg-gray-50 rounded-xl p-6 mb-6 space-y-4">
              <h4 className="font-semibold text-gray-800">Add New Batch</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBatchForm(prev => ({...prev, schedule: e.target.value}))}
                    className="w-full p-3 border border-gray-300 rounded-lg"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBatchForm(prev => ({...prev, maxStudents: parseInt(e.target.value)}))}
                    className="w-full p-3 border border-gray-300 rounded-lg"
                    min="1"
                  />
                </div>
              ...
              <div className="flex gap-2">
                <button onClick={addBatch} className="bg-green-600 text-white px-4 py-2 rounded-lg">
                  Add Batch
                </button>
                <button onClick={() => setShowBatchForm(false)} className="bg-gray-500 text-white px-4 py-2 rounded-lg">
                  Cancel
                </button>
              </div>
            </div>
          ...
          <div className="space-y-4">
            {batches.map((batch) => (
              <div key={batch.id} className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:shadow-md transition-shadow">
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
                    ...
                    <div>👥 {batch.enrolledStudents}/{batch.maxStudents}</div>
                    <div>👨‍🏫 {batch.instructor || "TBA"}</div>
                  </div>
                </div>
                <button
          ...
          {batches.length === 0 && (
            <div className="text-center py-8 text-gray-500">
              No batches created yet. Add your first batch to get started.
            </div>
          )}
          ...
          <div className="flex gap-3 pt-6 border-t border-gray-200 mt-6">
            <button
              onClick={saveBatches}
              className="flex-1 bg-gradient-to-r from-indigo-500 to-purple-600 text-white p-4 rounded-xl font-medium hover:shadow-lg transition-all"
            >
              Save Batches
            </button>
            <button
...
const CourseManagement = () => {
  ...
  const handleSaveCourse = async (e) => {
    ...
    try {
      ...
      const courseData = {
        ...
        batches: editingCourse ? (editingCourse.batches || []) : [],
        updatedAt: new Date().toISOString(),
        status: "active",
  ...
  const stats = {
    ...
    totalBatches: courses.reduce((sum, course) => sum + (course.batches?.length || 0), 0),
    ongoingBatches: courses.reduce((sum, course) => sum + (course.batches?.filter(b => b.status === 'ongoing').length || 0), 0)
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2 text-lg">
              Create, manage, and track your courses and batches
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            ...
            { label: "Total Batches", value: stats.totalBatches, icon: "👥", color: "from-purple-500 to-pink-600" },
            { label: "Active Batches", value: stats.ongoingBatches, icon: "🔥", color: "from-green-500 to-emerald-600" },
            { label: "Total Students", value: stats.totalStudents, icon: "🎓", color: "from-orange-500 to-red-600" }
          ].map((stat, index) => (
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSaveCourse} className="p-6 space-y-6">
                {/* ... (Keep the same form content as before) */}
                {activeTab === 'overview' && (
                  <>
